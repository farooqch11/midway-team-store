...
module StoreFrontHelper
  ...
  class MyCustomLinkRenderer < WillPaginate::ViewHelpers::LinkRendererBase
    ...
    def container_attributes
      ...
      }.update @options.except(*(WillPaginate::ViewHelpers.pagination_options.keys + [:renderer] - [:class]))
    end
    ...
    def url(page)
      return "#{@options[:pre_path]}?#{@template.params.permit!().merge({ :p => page }).slice(:category, :filter, :p, :per_page).to_h.to_a.map { |x| "#{x[0]}=#{x[1]}" }.join("&")}"
    end
